<x-app>
    ...
    <div class="card shadow-lg p-3">
        <form action="{{ route('loan.store') }}" method="post" class="form">
            @csrf
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select select2-default @error('member_id') is-invalid  @enderror" id="member_id" name="member_id" required>
                            ...
                            @foreach ($members as $member)
                                <option value="{{ $member->id }}" @selected(old('member_id') == $member->id)>{{ $member->user->name }} ({{ $member->member_number }})</option>
                            @endforeach
                        </select>
                        @error('member_id')
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('amount') is-invalid  @enderror" type="number" min="100000" id="amount" name="amount" required value="{{ old('amount') }}">
                        @error('amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('tenor') is-invalid  @enderror" type="number" min="1" id="tenor" name="tenor" required value="{{ old('tenor') }}">
                        @error('tenor')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid  @enderror" id="status" name="status" required>
                            ...
                            <option value="Approved" @selected(old('status') == 'Approved')>Disetujui (Approved)</option>
                            <option value="Rejected" @selected(old('status') == 'Rejected')>Ditolak (Rejected)</option>
                            <option value="Active" @selected(old('status') == 'Active')>Aktif (Active)</option>
                            <option value="Paid_Off" @selected(old('status') == 'Paid_Off')>Lunas (Paid Off)</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('due_date') is-invalid  @enderror" type="date" id="due_date" name="due_date" value="{{ old('due_date') }}">
                        <small class="text-muted">Isi jika status Aktif</small>
                        @error('due_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
